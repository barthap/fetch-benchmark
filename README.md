...
All benchmarks are defined in [`benchmarks/index.ts`](./benchmarks/index.ts).
...
- **Localhost hosted** to mitigate networking impact _(some libraries do processing during the `await fetch()` call instead of `await response.<<desiredFormat>>()` so networking time could not be fully separated.)_
...
## Streaming benchmarks for `expo/fetch`
